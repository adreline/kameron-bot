...
manual.set('stacktrace','` kameron stacktrace <log file> `\nallows you to view log files in case something broke\n you can choose:\n-kameron (default if none is chosen)\n-cron\nexample:\n` kameron stacktrace cron `');
manual.set('me','` kameron me `\nprints your stats\n**feature is work in progress**');
manual.set('opgg','` kameron opgg `\n\nshows you current match info. If you are __not__ in match it will show you your profile.\n\n` kameron opgg <region> <summoner name> `\n\nthis will link your discord account with your lol account. you can change linked lol account anytime by just using this command again with new name and region.\n**example:**\n\n` kameron opgg euw Debaleba `');
exports.getHelp = function(command,client,thumbnail){
console.log('[manual.js] Manual getHelp() function called');
